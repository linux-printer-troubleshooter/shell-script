...
remove_printer () {
 echo Listing Printers..........;ass=$( lpstat -e |  zenity --list --column="Job ID" --column="User" --column="Queue" --column=File  --width=400 --height=700 --text="Printers" --title="$stats" --extra-button="URI"  --ok-label="Done"  2>/dev/null );page=$(echo $ass | awk '{print $1}');echo $ass;if [[ $page == 'URI' ]]; then if zenity --question --text="Open URI?"; then troubleshoot_device ;fi;else lpadmin -x $ass;fi;
}
...
troubleshoot_device () { brand=$(zenity --entry --entry-text=HP --text="Enter printer Manuacturer name\nNote:- this Keyword is used for searching only.\nHP hewlett packward.\nType lpinfo -m in the terminal\nto get a list of models as stored by Linux Os" --width=100 );uri_list=$(lpinfo -l -v | grep -i -B 4 -C 2 "make-and-model = "$brand | grep "uri =" | awk -F "uri = " '{print $2 }' | zenity --text="Listing Printer Uri\nCopy to clipboard" --height=400 --width=400 --title="Printer ID" --list --column=DeviceURI --ok-label="Copy" 2>/dev/null);echo $uri_list | xclip -selection clipboard ;} ;open_queue; while  zenity --question --text="Refresh Job queue?" --width=70;do open_queue;done
